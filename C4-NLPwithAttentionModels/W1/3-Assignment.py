...
def input_encoder_fn(input_vocab_size, d_model, n_encoder_layers):
    ...
    input_encoder = tl.Serial( 
        ...
        tl.Embedding(input_vocab_size,d_model),  ##None,
        
        # feed the embeddings to the LSTM layers. It is a stack of n_encoder_layers LSTM layers
        [tl.LSTM(d_model) for _ in range(n_encoder_layers)]    ##None
        ### END CODE HERE ### 
    )
...
def pre_attention_decoder_fn(mode, target_vocab_size, d_model):
    ...
    pre_attention_decoder = tl.Serial(
        ...
        tl.ShiftRight(mode=mode),  ##None,

        # run an embedding layer to convert tokens to vectors
        tl.Embedding(target_vocab_size, d_model),    ###None,

        # feed to an LSTM layer
        tl.LSTM(d_model)   ##None
        ### END CODE HERE ###
    )
...
def prepare_attention_input(encoder_activations, decoder_activations, inputs):
    ...
    keys = encoder_activations   ##None
    values = encoder_activations   ##None

    
    # set the queries to the decoder activations
    queries = decoder_activations   ##None
...
def NMTAttn(input_vocab_size=33300,
    ...
    input_encoder = input_encoder_fn(input_vocab_size, d_model,n_encoder_layers)   ##None

    # Step 0: call the helper function to create layers for the pre-attention decoder
    pre_attention_decoder = pre_attention_decoder_fn(mode, target_vocab_size, d_model)   ##None

    # Step 1: create a serial network
    model = tl.Serial( 
        
      # Step 2: copy input tokens and target tokens as they will be needed later.
      tl.Select([0,1,0,1]),  ##None,
        
      # Step 3: run input encoder on the input and pre-attention decoder the target.
      tl.Parallel(input_encoder, pre_attention_decoder),  ##None(None, None),
        
      # Step 4: prepare queries, keys, values and mask for attention.
      tl.Fn('PrepareAttentionInput', prepare_attention_input, n_out=4),   ##None,
        
      # Step 5: run the AttentionQKV layer
      # nest it inside a Residual layer to add to the pre-attention decoder activations(i.e. queries)
      tl.Residual(tl.AttentionQKV(d_model, n_heads=n_attention_heads, dropout=attention_dropout, mode=None)),  ##None
      ...
      tl.Dense(target_vocab_size),   ##None
...
train_task = training.TrainTask(
    ...
    labeled_data= train_batch_stream,  ##None,
    ...
    lr_schedule= trax.lr.warmup_and_rsqrt_decay(1000, 0.01),  ##None,
...
def next_symbol(NMTAttn, input_tokens, cur_output_tokens, temperature):
    ...
    token_length = len(cur_output_tokens)  ##None

    # calculate next power of 2 for padding length 
    padded_length = 2**int(np.ceil(np.log2(token_length + 1)))  ##None

    # pad cur_output_tokens up to the padded_length
    padded = cur_output_tokens + [0] * (padded_length - token_length)  ##None
    ...
    padded_with_batch = np.expand_dims(padded, axis=0)   ##None
    ...
    output, _ = NMTAttn((input_tokens, padded_with_batch))   ##None
    
    # get log probabilities from the last token output
    log_probs = output[0, token_length, :]    ##None]

    # get the next symbol by getting a logsoftmax sample (*hint: cast to an int)
    symbol = int(tl.logsoftmax_sample(log_probs, temperature))   ##None
...
def sampling_decode(input_sentence, NMTAttn = None, temperature=0.0, vocab_file=None, vocab_dir=None):
    ...
    input_tokens = tokenize(input_sentence, vocab_file, vocab_dir)   ##None
    ...
    while cur_output != EOS:
        ...
        cur_output, log_prob = next_symbol(NMTAttn,input_tokens,cur_output_tokens,temperature)   ##None
        
        # append the current output token to the list of output tokens
        cur_output_tokens.append(cur_output)
    
    # detokenize the output tokens
    sentence = detokenize(cur_output_tokens, vocab_file, vocab_dir)   ##None
...
def rouge1_similarity(system, reference):
    ...
    for token in sys_counter:
        ...
        token_count_sys = sys_counter.get(token,0)  ##None
        
        # lookup the value of the token in the ref_counter dictionary (hint: use the get() method)
        token_count_ref = ref_counter.get(token,0)    ##None
        
        # update the overlap by getting the smaller number between the two token counts above
        overlap += min(token_count_sys,token_count_ref)   ##None
    
    # get the precision (i.e. number of overlapping tokens / number of system tokens)
    precision = overlap/sum(sys_counter.values())   ##None
    
    # get the recall (i.e. number of overlapping tokens / number of reference tokens)
    recall = overlap / sum(ref_counter.values())  ##None
    
    if precision + recall != 0:
        # compute the f1-score
        rouge1_score = 2*((precision*recall)/(precision+recall))  ##None
    else:
        rouge1_score = 0 
    ### END CODE HERE ###
...
def average_overlap(similarity_fn, samples, *ignore_params):
    ...
    for index_candidate, candidate in enumerate(samples):    
        ...
        for index_sample, sample in enumerate(samples): 
            ...
            sample_overlap = similarity_fn(candidate,sample)  ##None
            
            # add the sample overlap to the total overlap
            overlap += sample_overlap  ##None
            
        # get the score for the candidate by computing the average
        score = overlap/index_sample  ##None
        
        # save the score in the dictionary. use index as the key.
        scores[index_candidate] = score   ##None
...
def mbr_decode(sentence, n_samples, score_fn, similarity_fn, NMTAttn=None, temperature=0.6, vocab_file=None, vocab_dir=None):
    ...
    ### START CODE HERE (REPLACE INSTANCES OF `None` WITH YOUR CODE) ###
    # generate samples
    samples, log_probs = generate_samples(sentence, n_samples, NMTAttn, temperature, vocab_file, vocab_dir)   ##None
    
    # use the scoring function to get a dictionary of scores
    # pass in the relevant parameters as shown in the function definition of 
    # the mean methods you developed earlier
    scores = weighted_avg_overlap(jaccard_similarity,samples,log_probs)   ##None
    
    # find the key with the highest score
    max_index = max(scores,key=scores.get)  ##None
    
    # detokenize the token list associated with the max_index
    translated_sentence = detokenize(samples[max_index],vocab_file,vocab_dir)   ##None
