...
def get_conversation(file, data_db):
    ...
    ### START CODE HERE (REPLACE INSTANCES OF 'None' WITH YOUR CODE) ###
        ...
        cur_log = data_db[file]['log'][i]  ##None
        
        # check if i is even
        if i%2==0:  ##None:                   
            # append the 1st delimiter string
            result += delimiter_1  ##None
        else: 
            # append the 2nd delimiter string
            result += delimiter_2  ##None
        
        # append the message text from the log
        result += cur_log['text']  ##None
...
def reversible_layer_reverse(y, f, g):
    ...
    x = np.concatenate([x1,x2],axis=-1)  ##None
...
def ReformerLM_output_gen(ReformerLM, start_sentence, vocab_file, vocab_dir, temperature):
    ...
    input_tokens_with_batch = np.array(input_tokens)[None, :]  ##None
    ...
    output_gen = trax.supervised.decoding.autoregressive_sample_stream( 
        ...
        inputs=input_tokens_with_batch,  ##None,
        # temperature
