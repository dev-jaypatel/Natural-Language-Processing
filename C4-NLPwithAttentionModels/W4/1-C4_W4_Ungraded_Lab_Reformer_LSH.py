...
def our_simple_attend(
  ...
  """
    ...
    dots, dots_logsumexp = our_softmax(dots)   ##None
    if verbose:
...
def our_hash_vectors(vecs, rng, n_buckets, n_hashes, mask=None, verbose=False):
  ...
  Returns:
    ...
    ### Step 1 ###
    rotations_shape = (vecs.shape[-1], n_hashes, rot_size // 2)   ##None
    ...
    if fastmath.backend_name() == "jax":
        ...
        random_rotations = np.reshape(random_rotations,
                                    [-1, n_hashes * (rot_size // 2)])  ##None
        if verbose:
            print("random_rotations reshaped", random_rotations.shape)
        # Step 2b
        rotated_vecs = np.dot(vecs, random_rotations)  ##None
        if verbose:
            print("rotated_vecs1", rotated_vecs.shape)
        # Step 2c
        rotated_vecs = np.reshape(rotated_vecs, [-1, n_hashes, rot_size//2])   ##None
        if verbose:
            print("rotated_vecs2", rotated_vecs.shape)
        # Step 2d
        rotated_vecs = np.transpose(rotated_vecs, (1, 0, 2))  ##None
        if verbose:
            print("rotated_vecs3", rotated_vecs.shape)

    ### Step 3 ###
    rotated_vecs = np.concatenate([rotated_vecs, -rotated_vecs], axis=-1)  ##None
    if verbose:
        print("rotated_vecs.shape", rotated_vecs.shape)
    ### Step 4 ###
    buckets = np.argmax(rotated_vecs, axis=-1).astype(np.int32)  ##None
    if verbose:
    ...
    ### Step 5 ###
    buckets = np.reshape(buckets + offsets, (-1,))  ##None
    if verbose:
...
def sort_buckets(buckets, q, v, n_buckets, n_hashes, seqlen, verbose=True):
  ...
  Args:
    ...
    if verbose:
        print("---sort_buckets--")
    ## Step 1
    ticker = np.arange(n_hashes * seqlen)  ##None
    if verbose:
    ...
    # Step 3
    sbuckets_and_t, sticker = fastmath.sort_key_val(buckets_and_t, ticker, dimension=-1)  ##None
    if verbose:  ##
        print("sbuckets_and_t", sbuckets_and_t.shape, sbuckets_and_t)
    if verbose:
        print("sticker", sticker.shape, sticker)
    # Step 4
    _, undo_sort = fastmath.sort_key_val(sticker, ticker, dimension=-1)  ##None
    if verbose:
        print("undo_sort", undo_sort.shape, undo_sort)

    # Step 5
    st = sticker % seqlen  # provided
    sq = np.take(q, st, axis=0)  ##None
    sv = np.take(v, st, axis=0)  ##None
...
def dotandv(
    ...
    # Step 1  
    rsq = np.reshape(sq,(-1, kv_chunk_len, sq.shape[-1]))  ##None
    rsqt = np.swapaxes(rsq, -1, -2)  ##None
    if verbose:
        print("rsq.shape,rsqt.shape: ", rsq.shape, rsqt.shape)
    dotlike = np.matmul(rsq, rsqt)  ##None
    if verbose:
        print("dotlike\n", dotlike)

    # Step 2
    dotlike, slogits = our_softmax(dotlike, passthrough)  ##None
    if verbose:
        print("dotlike post softmax\n", dotlike)

    # Step 3
    vr = np.reshape(sv, (-1, kv_chunk_len, sv.shape[-1]))  ##None
    if verbose:
        print("dotlike.shape, vr.shape:", dotlike.shape, vr.shape)
    so = np.matmul(dotlike, vr)  ##None
    if verbose:
        print("so.shape:", so.shape)
    so = np.reshape(so, (-1, so.shape[-1]))   ##None  
    slogits = np.reshape(slogits, (-1,))  # provided
    if verbose:
        print("so.shape,slogits.shape", so.shape, slogits.shape)

    # Step 4
    o = np.take(so, undo_sort, axis=0)  ##None
    logits = np.take(slogits, undo_sort, axis=0)  ##None
    if verbose:
